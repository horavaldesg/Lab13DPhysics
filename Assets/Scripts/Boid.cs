...
public class Boid : MonoBehaviour
{
    ...
    public enum BoidType{Lead, Support };
    ...
    void Awake()
    {
        ...
        if(boidType != BoidType.Lead) return; // Sets the lead to color white
        SetColor(Color.white);
    }

   

    private void SetColor(Color randColor)
    {
    ...
    void LookAhead()
    {
        ...
        //var lookRot = Quaternion.LookRotation(pos + rigid.velocity.normalized);
        
        Vector3 tvec;
        tvec = brot * Vector3.up;
        Quaternion qtemp = Quaternion.LookRotation(pos + rigid.velocity, tvec);
      //  transform.rotation = Quaternion.Slerp(transform.rotation, qtemp, 0.2f);
    ...
    private void FixedUpdate()
    {
        ...
        var tooClosePosObstacle = collisionAvoidance.avgPosCol;
        if (tooClosePosObstacle != Vector3.zero)
        {
            velAvoid = pos - tooClosePosObstacle;
            velAvoid.Normalize();
            velAvoid *= spn.velocity;
        }
        ...
        var delta_myangle = rotationRate * Time.deltaTime; //update the angle step
    ...
    private void AvoidCollission(bool val)
    {
        _avoidCol = val;
    }
